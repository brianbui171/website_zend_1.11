<?php 
    ...
    if($this->arrParams['action'] == 'add'){
        ...
        $linkSaveItem    = $this->baseUrl($this->currentController . '/edit/id/' . $this->item['category_id']);
    }
    ...
    //9. Edit Item - link
    $linkEditItem        = $this->baseUrl($this->currentController . '/edit/id/' . $this->item['category_id']);
    ...
    switch ($this->arrParams['action']) {
        case 'index':
        $strBtn = $btnActiveItems . ' ' . $btnInactiveItems . ' ' . $btnSortItems . ' ' . $btnAddItems . ' ' . $btnDeleteItems;
        break;
        case 'add':
