...
abstract class Zend_Translate_Adapter {
	 ...
	 * @var array
	 */
	protected $_options = array ('clear' => false, 'content' => null, 'disableNotices' => false, 'ignore' => '.', 'locale' => 'auto', 'log' => null, 'logMessage' => "Untranslated message within '%locale%': %message%", 'logUntranslated' => false, 'reload' => false, 'route' => null, 'scan' => null );
	...
	public function addTranslation($options = array()) {
		...
		if (is_string ( $options ['content'] ) and is_dir ( $options ['content'] )) {
			...
			$prev = '';
			foreach ( new RecursiveIteratorIterator ( new RecursiveDirectoryIterator ( $options ['content'], RecursiveDirectoryIterator::KEY_AS_PATHNAME ), RecursiveIteratorIterator::SELF_FIRST ) as $directory => $info ) {
