...
class Zend_Service_WindowsAzure_Storage_TableEntity {
	...
	public function getAzureValues() {
		...
		foreach ( $accessors as $accessor ) {
			if ($accessor->EntityType == 'ReflectionProperty') {
				...
				$returnValue [] = ( object ) array ('Name' => $accessor->AzurePropertyName, 'Type' => $accessor->AzurePropertyType, 'Value' => $this->$property );
				...
				$returnValue [] = ( object ) array ('Name' => $accessor->AzurePropertyName, 'Type' => $accessor->AzurePropertyType, 'Value' => $this->$method () );
			}
		}
	...
	public static function getAzureAccessor($member) {
		...
		return ( object ) array ('EntityAccessor' => $member->getName (), 'EntityType' => get_class ( $member ), 'AzurePropertyName' => $azureProperties [0], 'AzurePropertyType' => isset ( $azureProperties [1] ) ? $azureProperties [1] : '' );
	}
}
