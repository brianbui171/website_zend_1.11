...
class Zend_Service_LiveDocx_MailMerge extends Zend_Service_LiveDocx {
	...
	public function setLocalTemplate($filename) {
		...
		try {
			$this->getSoapClient ()->SetLocalTemplate ( array ('template' => base64_encode ( file_get_contents ( $filename ) ), 'format' => self::getFormat ( $filename ) ) );
		} catch ( Exception $e ) {
	...
	public function setRemoteTemplate($filename) {
		...
		try {
			$this->getSoapClient ()->SetRemoteTemplate ( array ('filename' => $filename ) );
		} catch ( Exception $e ) {
			require_once 'Zend/Service/LiveDocx/Exception.php';
	...
	public function setFieldValues($values) {
		...
		try {
			$this->getSoapClient ()->SetFieldValues ( array ('fieldValues' => self::$method ( $values ) ) );
		} catch ( Exception $e ) {
			require_once 'Zend/Service/LiveDocx/Exception.php';
	...
	public function setBlockFieldValues($blockName, $blockFieldValues) {
		...
		try {
			$this->getSoapClient ()->SetBlockFieldValues ( array ('blockName' => $blockName, 'blockFieldValues' => self::multiAssocArrayToArrayOfArrayOfString ( $blockFieldValues ) ) );
		} catch ( Exception $e ) {
			require_once 'Zend/Service/LiveDocx/Exception.php';
	...
	public function setDocumentPassword($password) {
		...
		try {
			$this->getSoapClient ()->SetDocumentPassword ( array ('password' => $password ) );
		} catch ( Exception $e ) {
			require_once 'Zend/Service/LiveDocx/Exception.php';
	...
	public function setDocumentAccessPermissions($permissions, $password) {
		...
		try {
			$this->getSoapClient ()->SetDocumentAccessPermissions ( array ('permissions' => $permissions, 'password' => $password ) );
		} catch ( Exception $e ) {
			require_once 'Zend/Service/LiveDocx/Exception.php';
	...
	public function retrieveDocument($format) {
		...
		try {
			$result = $this->getSoapClient ()->RetrieveDocument ( array ('format' => $format ) );
		} catch ( Exception $e ) {
			require_once 'Zend/Service/LiveDocx/Exception.php';
	...
	public function getMetafiles($fromPage, $toPage) {
		...
		$ret = array ();
		$result = $this->getSoapClient ()->GetMetafiles ( array ('fromPage' => ( integer ) $fromPage, 'toPage' => ( integer ) $toPage ) );
	...
	public function getBitmaps($fromPage, $toPage, $zoomFactor, $format) {
		...
		$result = $this->getSoapClient ()->GetBitmaps ( array ('fromPage' => ( integer ) $fromPage, 'toPage' => ( integer ) $toPage, 'zoomFactor' => ( integer ) $zoomFactor, 'format' => ( string ) $format ) );
	...
	public function getAllBitmaps($zoomFactor, $format) {
		...
		$ret = array ();
		$result = $this->getSoapClient ()->GetAllBitmaps ( array ('zoomFactor' => ( integer ) $zoomFactor, 'format' => ( string ) $format ) );
	...
	public function getBlockFieldNames($blockName) {
		...
		$ret = array ();
		$result = $this->getSoapClient ()->GetBlockFieldNames ( array ('blockName' => $blockName ) );
	...
	public function uploadTemplate($filename) {
		...
		try {
			$this->getSoapClient ()->UploadTemplate ( array ('template' => base64_encode ( file_get_contents ( $filename ) ), 'filename' => basename ( $filename ) ) );
		} catch ( Exception $e ) {
			require_once 'Zend/Service/LiveDocx/Exception.php';
	...
	public function downloadTemplate($filename) {
		...
		try {
			$result = $this->getSoapClient ()->DownloadTemplate ( array ('filename' => basename ( $filename ) ) );
		} catch ( Exception $e ) {
			require_once 'Zend/Service/LiveDocx/Exception.php';
	...
	public function deleteTemplate($filename) {
		...
		$this->getSoapClient ()->DeleteTemplate ( array ('filename' => basename ( $filename ) ) );
	}
	...
	public function templateExists($filename) {
		...
		$result = $this->getSoapClient ()->TemplateExists ( array ('filename' => basename ( $filename ) ) );
	...
	public function deleteSharedDocument($filename) {
		...
		$this->getSoapClient ()->DeleteSharedDocument ( array ('filename' => basename ( $filename ) ) );
	}
	...
	public function downloadSharedDocument($filename) {
		...
		try {
			$result = $this->getSoapClient ()->DownloadSharedDocument ( array ('filename' => basename ( $filename ) ) );
		} catch ( Exception $e ) {
			require_once 'Zend/Service/LiveDocx/Exception.php';
		...
		if (isset ( $list->ArrayOfString )) {
			foreach ( $list->ArrayOfString as $a ) {
				...
				if (isset ( $o->string )) {
					...
					$ret [] = array ('filename' => $o->string [0], 'fileSize' => ( integer ) $o->string [2], 'createTime' => ( integer ) $date1->get ( Zend_Date::TIMESTAMP ), 'modifyTime' => ( integer ) $date2->get ( Zend_Date::TIMESTAMP ) );
				}
			}
		}
