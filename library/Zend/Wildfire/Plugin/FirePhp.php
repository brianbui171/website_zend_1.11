...
class Zend_Wildfire_Plugin_FirePhp implements Zend_Wildfire_Plugin_Interface {
	...
	public static function send($var, $label = null, $style = null, $options = array()) {
		...
		if ($var instanceof Exception) {
			...
			$var = array ('Class' => get_class ( $var ), 'Message' => $var->getMessage (), 'File' => $var->getFile (), 'Line' => $var->getLine (), 'Type' => 'throw', 'Trace' => $firephp->_encodeTrace ( $eTrace ) );
			...
			$var = array ('Class' => $trace [0] ['class'], 'Type' => $trace [0] ['type'], 'Function' => $trace [0] ['function'], 'Message' => $label, 'File' => isset ( $trace [0] ['file'] ) ? $trace [0] ['file'] : '', 'Line' => isset ( $trace [0] ['line'] ) ? $trace [0] ['line'] : '', 'Args' => isset ( $trace [0] ['args'] ) ? $firephp->_encodeObject ( $trace [0] ['args'] ) : '', 'Trace' => $firephp->_encodeTrace ( array_splice ( $trace, 1 ) ) );
		...
		if ($meta ['Type'] == self::DUMP) {
			...
			return $firephp->_recordMessage ( self::STRUCTURE_URI_DUMP, array ('key' => $meta ['Label'], 'data' => $var ), $skipFinalEncode );
		
		} else {
			
			return $firephp->_recordMessage ( self::STRUCTURE_URI_FIREBUGCONSOLE, array ('data' => $var, 'meta' => $meta ), $skipFinalEncode );
		}
	}
	...
	protected function _recordMessage($structure, $data, $skipEncode = false) {
		switch ($structure) {
			...
			case self::STRUCTURE_URI_DUMP :
				...
				return $this->_channel->getProtocol ( self::PROTOCOL_URI )->recordMessage ( $this, $structure, array ($data ['key'] => $value ) );
			...
			case self::STRUCTURE_URI_FIREBUGCONSOLE :
				...
				return $this->_channel->getProtocol ( self::PROTOCOL_URI )->recordMessage ( $this, $structure, array ($data ['meta'], $value ) );
