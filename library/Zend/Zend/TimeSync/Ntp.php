...
class Zend_TimeSync_Ntp extends Zend_TimeSync_Protocol {
	...
	protected function _read() {
		...
		$result = array ('flags' => $flags, 'stratum' => ord ( fread ( $this->_socket, 1 ) ), 'poll' => ord ( fread ( $this->_socket, 1 ) ), 'precision' => ord ( fread ( $this->_socket, 1 ) ), 'rootdelay' => $this->_getFloat ( fread ( $this->_socket, 4 ) ), 'rootdispersion' => $this->_getFloat ( fread ( $this->_socket, 4 ) ), 'referenceid' => fread ( $this->_socket, 4 ), 'referencestamp' => $this->_getTimestamp ( fread ( $this->_socket, 8 ) ), 'originatestamp' => $this->_getTimestamp ( fread ( $this->_socket, 8 ) ), 'receivestamp' => $this->_getTimestamp ( fread ( $this->_socket, 8 ) ), 'transmitstamp' => $this->_getTimestamp ( fread ( $this->_socket, 8 ) ), 'clientreceived' => microtime ( true ) );
