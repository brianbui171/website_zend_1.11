...
class Zend_Feed_Reader_Extension_Atom_Entry extends Zend_Feed_Reader_Extension_EntryAbstract {
	...
	protected function _collectXhtml($xhtml, $prefix) {
		if (! empty ( $prefix ))
			$prefix = $prefix . ':';
		$matches = array ("/<\?xml[^<]*>[^<]*<" . $prefix . "div[^<]*/", "/<\/" . $prefix . "div>\s*$/" );
	...
	public function getCategories() {
		...
		if ($list->length) {
			...
			foreach ( $list as $category ) {
				$categoryCollection [] = array ('term' => $category->getAttribute ( 'term' ), 'scheme' => $category->getAttribute ( 'scheme' ), 'label' => $category->getAttribute ( 'label' ) );
			}
		} else {
