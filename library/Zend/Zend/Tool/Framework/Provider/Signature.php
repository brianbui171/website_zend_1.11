...
class Zend_Tool_Framework_Provider_Signature implements Zend_Tool_Framework_Registry_EnabledInterface {
	...
	protected function _processActionableMethods() {
		...
		foreach ( $methods as $method ) {
			 ...
			 * contain the name "
			 */
			if (! $method->getDeclaringClass ()->isInstantiable () || ! $method->isPublic () || $methodName [0] == '_' || $method->isStatic () || in_array ( $methodName, array ('getContextClasses', 'getName' ) )) // other protected public methods will nee to go here
{
				continue;
			}
