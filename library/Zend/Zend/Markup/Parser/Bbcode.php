...
class Zend_Markup_Parser_Bbcode implements Zend_Markup_Parser_ParserInterface {
	 ...
	 * @var array
	 */
	protected $_tags = array ('Zend_Markup_Root' => array ('type' => self::TYPE_DEFAULT, 'stoppers' => array () ), '*' => array ('type' => self::TYPE_DEFAULT, 'stoppers' => array (self::NEWLINE, '[/*]', '[/]' ) ), 'hr' => array ('type' => self::TYPE_SINGLE, 'stoppers' => array () ), 'code' => array ('type' => self::TYPE_DEFAULT, 'stoppers' => array ('[/code]', '[/]' ), 'parse_inside' => false ) );
		...
		while ( $this->_pointer < $this->_valueLen ) {
			switch ($this->_state) {
				case self::STATE_SCAN :
					...
					$this->_temp = array ('tag' => '[' . $matches ['name'], 'name' => $matches ['name'], 'attributes' => array () );
	...
	protected function _checkTagDeclaration($name) {
		if (! isset ( $this->_tags [$name] )) {
			$this->_tags [$name] = array ('type' => self::TYPE_DEFAULT, 'stoppers' => array ('[/' . $name . ']', '[/]' ) );
		}
	}
	/**
