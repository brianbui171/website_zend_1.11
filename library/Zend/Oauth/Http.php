...
empty ( $body )) // Missing token
{
	...
	protected function _assessRequestAttempt(Zend_Http_Response $response = null) {
		switch ($this->_preferredRequestScheme) {
			...
			default :
				require_once 'Zend/Oauth/Exception.php';
				throw new Zend_Oauth_Exception ( 'Could not retrieve a valid Token response from Token URL:' . (! is_null ( $response ) ? PHP_EOL . $response->getBody () : ' No body - check for headers') );
		}
	}
