...
class Zend_Oauth_Client extends Zend_Http_Client {
	...
	public function prepareOauth() {
		...
		if ($requestScheme == Zend_Oauth::REQUEST_SCHEME_HEADER) {
			$oauthHeaderValue = $this->getToken ()->toHeader ( $this->getUri ( true ), $this->_config, $this->_getSignableParametersAsQueryString () );
			...
			if ($query) {
				...
				foreach ( $queryParts as $queryPart ) {
					...
					$params [$kvTuple [0]] = (array_key_exists ( 1, $kvTuple ) ? $kvTuple [1] : NULL);
				}
			}
