...
function getFileUrl($value) {
	...
	if ($pos !== false && $pos == 0) {
		...
		//$output = $value;
		$output =substr($value,strlen('D:/vhosts/mamnonthandong.edu.vn/httpdocs/'));
	}
...
if (! function_exists ( "imagerotate" )) {
	function imagerotate($src_img, $angle, $bicubic = false) {
		...
		for($y = 0; $y < $src_y; $y ++) {
			for($x = 0; $x < $src_x; $x ++) {
				// rotate...
				$old_x = (($center_x - $x) * $cosangle + ($center_y - $y) * $sinangle) + $center_x;
				$old_y = (($center_y - $y) * $cosangle - ($center_x - $x) * $sinangle) + $center_y;
