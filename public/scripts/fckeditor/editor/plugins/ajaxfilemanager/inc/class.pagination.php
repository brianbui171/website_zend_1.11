...
class pagination {
	...
	function getLastUrl() {
		...
		if ($this->currentPage < $totalPages) {
			...
			$url = sprintf ( '<a href="%s" class="pagination_last"><span>%s</span></a>', $url, $this->lastText );
		}
		return $url;
	}
	...
	function getFirstUrl() {
		...
		if ($this->currentPage > 1) {
			...
			$url = sprintf ( '<a href="%s" class="pagination_first"><span>%s</span></a>', $url, $this->firstText );
	...
	function getPreviousUrl() {
		...
		if ($this->currentPage > 1 && $this->totalItems > 0) {
			...
			$url = sprintf ( '<a href="%s" class="pagination_previous"><span>%s</span></a>', $url, $this->previousText );
	...
	function getNextUrl() {
		...
		if ($this->totalItems > ($this->currentPage * $this->itemsPerPage)) {
			...
			$url = sprintf ( '<a href="%s" class="pagination_next"><span>%s</span></a>', $url, $this->nextText );
		}
		return $url;
